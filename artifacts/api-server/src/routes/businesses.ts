...
router.post("/businesses", requireAuth, async (req, res): Promise<void> => {
  ...
  const [existing] = await db.select().from(businessesTable).where(eq(businessesTable.clientId, parseInt(clientId)));
  if (existing) {
    res.status(409).json({ error: "This client already has a business profile. Use PATCH to update it." });
    return;
  }
...
router.get("/clients/:id/business", requireAuth, async (req, res): Promise<void> => {
  const clientId = parseId(req.params.id);
  const [business] = await db.select().from(businessesTable).where(eq(businessesTable.clientId, clientId));
  if (!business) { res.status(404).json({ error: "No business profile for this client" }); return; }
  res.json(formatBusiness(business));
});
